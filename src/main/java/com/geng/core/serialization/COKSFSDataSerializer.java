...
public class COKSFSDataSerializer implements ISFSDataSerializer {
	...
	private static COKSFSDataSerializer instance = new COKSFSDataSerializer();
	private static ObjectMapper objectMapper = new ObjectMapper();
	private static int BUFFER_CHUNK_SIZE = 512;
	private final Logger logger;

	public static COKSFSDataSerializer getInstance() {
		return instance;
	}

	COKSFSDataSerializer() {
	...
	public ISFSArray binary2array(byte[] data) {
		if (data.length < 3) {
			throw new IllegalStateException(
					"Can't decode an SFSArray. Byte data is insufficient. Size: " + data.length + " bytes");
		}
	...
	private ISFSArray decodeSFSArray(ByteBuffer buffer) {
		...
		if (headerBuffer != SFSDataType.SFS_ARRAY.getTypeID()) {
			throw new IllegalStateException(
					"Invalid SFSDataType. Expected: " + SFSDataType.SFS_ARRAY.getTypeID() + ", found: " + headerBuffer);
		}
	...
	public ISFSObject binary2object(byte[] data) {
		if (data.length < 3) {
			throw new IllegalStateException(
					"Can't decode an SFSObject. Byte data is insufficient. Size: " + data.length + " bytes");
		}
		ByteBuffer buffer = ByteBuffer.allocate(data.length);
		buffer.put(data);
		buffer.flip();
	...
	private ISFSObject decodeSFSObject(ByteBuffer buffer) {
		...
		if (headerBuffer != SFSDataType.SFS_OBJECT.getTypeID()) {
			throw new IllegalStateException("Invalid SFSDataType. Expected: " + SFSDataType.SFS_OBJECT.getTypeID()
					+ ", found: " + headerBuffer);
		}
	...
	public ISFSArray json2array(String jsonStr) {
		if (jsonStr.length() < 2) {
			throw new IllegalStateException(
					"Can't decode SFSObject. JSON String is too short. Len: " + jsonStr.length());
		}
		try {
			Collection jsa = objectMapper.readValue(jsonStr, Collection.class);
			return decodeSFSArray(jsa);
		} catch (Exception e) {
			throw new IllegalStateException("(json2sfarray) Could not decode value for object: " + jsonStr, e);
		}
	}

	private ISFSArray decodeSFSArray(Collection jsa) {
		ISFSArray sfsArray = SFSArray.newInstance();
	...
	public ISFSObject json2object(String jsonStr) {
		...
		try {
			Map jso = objectMapper.readValue(jsonStr, Map.class);
			return decodeSFSObject(jso);
		} catch (IOException e) {
			throw new IllegalStateException("Can't decode SFSObject. json:" + jsonStr, e);
		}
	}

	private ISFSObject decodeSFSObject(Map jso) {
	...
	private SFSDataWrapper decodeJsonObject(Object o) {
		...
		if ((o instanceof Collection)) {
			return new SFSDataWrapper(SFSDataType.SFS_ARRAY, decodeSFSArray((Collection) o));
		}

		throw new IllegalArgumentException(
				String.format("Unrecognized DataType while converting JSONObject 2 SFSObject. Object: %s, Type: %s",
	...
	private ByteBuffer encodeObject(ByteBuffer buffer, SFSDataType typeId, Object object) {
		if (typeId == null) {
			throw new IllegalArgumentException(
					(new StringBuilder("Unrecognized type in SFSObject serialization: ")).append(typeId).toString());
		}
		switch (typeId) {
			case NULL:
	...
	private ByteBuffer binEncode_UTF_STRING_ARRAY(ByteBuffer buffer, Collection<String> value) {
		...
		for (int binStr = 0; binStr < arrayOfByte1; binStr++) {
			byte[] binItem = arrayOfByte2[binStr];
	...
	private void setObjectField(Object pojo, String fieldName, Object fieldValue) throws Exception {
		...
		if (isArray) {
			if (!(fieldValue instanceof Collection)) {
				throw new SFSRuntimeException("Problem during SFSObject => POJO conversion. Found array field in POJO: "
